...
/**
 * Simplifica un objeto de error para el logging
 * Elimina el stack trace y reduce la información a lo esencial
 */
  ...
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      try {
        ...
        // Registrar el error con información simplificada
        logger.error({
          err: simplifyError(error), // Usamos la función auxiliar para simplificar el error
          method: methodName,
          data: process.env.NODE_ENV !== 'production' ? data : undefined,
          duration: `${Date.now() - startTime}ms`
        }, `Error en ${methodName}`);
